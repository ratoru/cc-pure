...
pub fn main() !void {
    ...
    // 1. Read Input (Main Thread)
    // We must secure the data before threading
...
fn fetchGit(ctx: *GitContext) void {
    ...
    // Use catch to ensure thread doesn't panic main
...
fn calcUsage(ctx: *UsageContext, session: cc_pure.Session.Session) void {
    ...
    // We can use the session data directly now because it was passed in
